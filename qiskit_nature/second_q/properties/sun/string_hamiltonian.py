...
from .basic_sun_operators import get_generators_SU, psi_color, psidag_color
from ..wilson_fermions.hamiltonian import hamilton_hopp_mass_wilson, operator_sum
    ...
    for spinor_component in range(ncomponents):
        for alpha in range(ncolors):
            for beta in range(ncolors):
                summand = (
                    psidag_color(site, alpha, spinor_component, lattice, ncolors, ncomponents)
...
def string_hamilton_wilson(lattice, group_dim, rep, params, output="abstract"):
    ...
    mass_hopping_terms = hamilton_hopp_mass_wilson(
        lattice, rep=rep, params=params, output="abstract"
    )
    ...
    for color_idx in range(ncolors):
        for summand in mass_hopping_terms.operator_list:
            ...
            new_summand = BaseFermionOperator(
                "I" * n_ops_before
                + summand.label
                + "I" * (n_ops_total - n_ops_before - len(summand)),
                coeff=summand.coeff,
            )
