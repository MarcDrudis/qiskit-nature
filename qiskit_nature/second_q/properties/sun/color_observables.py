...
from .basic_sun_operators import psi_color, psidag_color
from .string_hamiltonian import conserved_charge
from ..wilson_fermions.observables import dirac, operator_sum
...
def color_site_mass(site, lattice, ncolors=2, rep=dirac, m=1.0):
    ...
    for color_idx in range(ncolors):
        for alpha in range(2):
            for beta in range(2):
                summands.append(
                    psidag_color(site, color_idx, alpha, lattice, ncolors)
                    * gamma0[alpha, beta]
...
def color_total_mass(lattice, ncolors=2, rep=dirac, m=1.0):
...
def color_site_charge(site, lattice, color_idx, ncolors=2, g=1.0):
...
def color_total_charge(lattice, color_idx, ncolors=2, g=1.0):
...
def gen_site_charge(site, lattice, generator, g=1.0):
...
def gen_total_charge(lattice, generator, g=1.0):
