...
def get_generators_SU(n, rep="fundamental"):
    ...
    """
    assert (
        isinstance(n, (int, np.integer)) and n > 0
    ), "The degree n of the group SU(n) must be a positive integer"
        ...
        for i in range(ngen):
            for j in range(i + 1, ngen):
                for k in range(j + 1, ngen):
                    # get coeff = 1.j * f^{ijk}
                    coeff = 2.0 * np.trace(commutator(fund_gen[i], fund_gen[j]) @ fund_gen[k])
...
def psi_color(site, color_component, spinor_component, lattice, ncolors, ncomponents=2):
    ...
    return BaseFermionOperator(
        "I" * n_ops_before + spinor_label + "I" * (n_ops_total - n_ops_before - spinor_length)
    )
